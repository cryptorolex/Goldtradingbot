...
// =====================================================================
// GoldTrendScalperV44 (2026-09-19) - LOGGING-ONLY build, no trading-logic change. Based on V43.
// Adds a PROBE line for each of the first 5 one-minute management bars of every cycle:
//   "PROBE n=<1..5> cyclePL=<$>" so the P/L path right after entry (acceptance vs rejection) can be
// joined to cycle outcomes for ALL cycles, including fast failures the 5-second VOL15 log misses.
// At defaults MUST reproduce V43/V42: net $20,383, PF 1.97, 760 rows, maxDD -$881.
//
...
namespace NinjaTrader.NinjaScript.Strategies
{
	...
	public class GoldTrendScalperV44 : Strategy
	{
		...
		private DateTime lastMgmtTime = DateTime.MinValue;
		private int      dupMgmtCount = 0;
		private int      probeN = 0;
		...
		protected override void OnStateChange()
		{
			...
			{
				Description   = "1-min gold scalper - V44 (2026-09-19): logging-only PROBE lines (cyclePL bars 1-5 after entry). No logic change. Baseline $20,383 / PF 1.97 / 760.";
				Name          = "GoldTrendScalperV44";
				...
				StallExitAppliesTo = StallExitScopeV44.Both;  // CONFIRMED-BEST (2026-08-24),
			...
			{
				LogLine("DUPMGMT total=" + dupMgmtCount);
		...
		protected override void OnBarUpdate()
		{
			...
			if (BarsInProgress == 3)
			{
				...
				{
					ExitLong("SessionExit", "");
					ExitShort("SessionExit", "");
			...
			if (!haltedForDay && today <= -DailyLossHalt)
			{
				haltedForDay = true;
				dayHaltKind  = "LOSS";
				ExitLong("DailyLossExit", ""); ExitShort("DailyLossExit", "");
				...
				&& (ToTime(Times[0][0]) / 100) >= FomcBlackoutFromTime)
			{
				if (Position.MarketPosition != MarketPosition.Flat)
				{
					ExitLong("FomcExit", ""); ExitShort("FomcExit", "");
			...
			bool   vBrkOk     = wantLong ? hh > double.MinValue / 2 : ll < double.MaxValue / 2;
			double vBrkMargin = (a > 0 && vBrkOk) ? (wantLong ? (Close[0] - hh) : (ll - Close[0])) / a : 0;
			double vRangePts  = High[0] - Low[0];
			double vCloseLoc  = vRangePts > 0 ? (wantLong ? (Close[0] - Low[0]) : (High[0] - Close[0])) / vRangePts : 0;
			string vCtx = " ADX=" + adx5[0].ToString("F1")
				...
				+ " brkMarginATR=" + vBrkMargin.ToString("F2")
				+ " closeLoc=" + vCloseLoc.ToString("F2")
		...
		private void UpdatePeakAndCheckStall(double px)
		{
			...
			bool stallScopeOK = StallExitAppliesTo == StallExitScopeV44.Both
				|| (isLong  && StallExitAppliesTo == StallExitScopeV44.LongsOnly)
				|| (!isLong && StallExitAppliesTo == StallExitScopeV44.ShortsOnly);
		...
		{
			DateTime mgmtT = BarsInProgress == 3 ? Times[3][0] : Times[0][0];
			if (mgmtT == lastMgmtTime) { dupMgmtCount++; LogLine(mgmtT.ToString("yyyy-MM-dd HH:mm") + "  DUPMGMT ManageCycle twice, BIP=" + BarsInProgress); }
			lastMgmtTime = mgmtT;
			bool   isLong  = Position.MarketPosition == MarketPosition.Long;
			string entSig  = !string.IsNullOrEmpty(cycleEntrySig) ? cycleEntrySig : (isLong ? "GL_C" : "GS");
			double cycleRealized = SystemPerformance.AllTrades.TradesPerformance.Currency.CumProfit - cumAtEntry;
			double cyclePL = cycleRealized + Position.GetUnrealizedProfitLoss(PerformanceUnit.Currency, px);
			if (probeN < 5) { probeN++; LogLine(mgmtT.ToString("yyyy-MM-dd HH:mm") + "  PROBE n=" + probeN + " cyclePL=" + cyclePL.ToString("F0")); }
		...
		public StallExitScopeV44 StallExitAppliesTo { get; set; }
